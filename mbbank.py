...
class MBBANK:
    def __init__(self,username, password, account_number, proxy_list=None):
        ...
        self.is_login = False
        self.file = f"data/mb/users/{username}.txt"
    ...
    def mapping_bank_code(self,bank_name):
        ...
        return None
    def check_bank_name_napas(self,ben_account_number, bank_name):
        ...
        if 'result' in response and 'responseCode' in response['result'] and response['result']['responseCode'] == "00" and 'benName' in response:
            response['customerName'] = response['benName']
    ...
    def get_bank_name(self,ben_account_number, bank_name):
        if not self.is_login or time.time() - self.time_login > 900:
            ...
            if not login['success']:
                return login
        result =  self.check_bank_name_napas(ben_account_number, bank_name)
        if 'customerName' not in result:
            login = self.handleLogin()
            if not login['success']:
                return login
        result =  self.check_bank_name_napas(ben_account_number, bank_name)
    ...
    def check_bank_name(self,ben_account_number, bank_name, ben_account_name):
        ...
        print('get_name_from_account_mb',get_name_from_account)
